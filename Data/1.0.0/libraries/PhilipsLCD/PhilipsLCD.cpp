...
void PhilipsLCD::LCDInit()	// Initialise the screen
{
LCDCommand(0x11);
LCDCommand(0x03);
LCDCommand(0x21);
...
void PhilipsLCD::LCDCommand(unsigned char data) // Send a hex command through the serial port (SDATA 0 first)
{
digitalWrite(CS,0); 	      // disable CS
    digitalWrite(SDATA,0);     // Disable Serial Data line, This means we are sending a command
    digitalWrite(SCLK,0);      // Disable Serial Clock
    digitalWrite(SCLK,1);	// Enable Serial Clock
    for (char j = 0; j < 8; j++)
    {
        if ((data & 0x80) == 0x80) 
    ...
    digitalWrite(SCLK,0);      // Disable Clock
    digitalWrite(SCLK,1);     // Enable Clock
        data <<= 1;
    }
digitalWrite(SCLK,0);      // Disable Clock
digitalWrite(SDATA,0);	   // Disable Serial Data	

digitalWrite(CS,1);     // Enable CS
}

void PhilipsLCD::LCDData(unsigned char data) // Send hex data through the serial port (SDATA 1 first)
{
digitalWrite(CS,0); 	      // disable CS
    digitalWrite(SDATA,1);     // Enable Serial Data line, This means we are sending data
    digitalWrite(SCLK,0);      // Disable Serial Clock
    digitalWrite(SCLK,1);	// Enable Serial Clock
    ...
    {
        if ((data & 0x80) == 0x80)
 digitalWrite(SDATA,1);	// Enable Serial Data
        else 
digitalWrite(SDATA,0); // Disable Serial Data line
